...
Этот мини-апп переделан с применением этого скилла. Направление — Editorial × Tech (Space Grotesk + Inter).`,
      },
      {
        ...
        excerpt: 'Шаблон ТЗ для генерации промпта текстового ИИ-продавца: структура Роль + Технология поведения, обязательные правила, воронка диалога.',
        date: dayBack(0),
        type: 'file',
        pinned: true,
        ...
        fileSize: '7 KB',
        body: `Готовый шаблон технического задания для генерации промпта ИИ-продавца, который ведёт переписку с лидами в мессенджерах (Telegram, WhatsApp, VK, Instagram-DM).

Под ChatGPT 5.1, GPT-4o, Claude Sonnet — любая модель уровня senior LLM.

Что внутри:
- Блок РОЛЬ — имя, должность, контекст обращений, источники трафика, маркетинг-обещания, стилистика
- Блок ТЕХНОЛОГИЯ ПОВЕДЕНИЯ — ключевая цель и универсальные правила: один вопрос за сообщение, не повторяться, не "присоединения", не выдавать что это ИИ
- Воронка диалога — установление контакта, выявление потребностей, дальнейшие этапы (заполняешь под свою нишу)

Как использовать:
1. Скачай файл по кнопке ниже.
2. Заполни пустые поля под свой бизнес (ниша, цель коммуникации, мессенджер, инфо о компании, обещания из рекламы).
3. Прокинь заполненный ТЗ в любую LLM с запросом "сгенерируй промпт по этому ТЗ".
4. Полученный промпт встрой в своего бота / агента / в FastAPI Worker.

Помни ключевые запреты в самом промпте:
- Запрещены примеры диалогов (только правила и смыслы)
- Запрещено описывать внешние интеграции (CRM/API/вебхуки)
- Запрещено "присоединения" вроде "понимаю", "спасибо за уточнение"
